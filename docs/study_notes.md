...
24. **What is Row Level Security (RLS)?** A database-level enforcement mechanism that filters rows based on the authenticated user's identity. Evaluated before data is returned, regardless of what SQL the application sent.
25. **Why is RLS "defense in depth" and not just a filter?** The application layer filters by producer_id in queries. RLS enforces it again independently at the database. An attacker who bypasses FastAPI still cannot read rows they don't own — they must defeat both layers independently.
26. **What is the difference between SUPABASE_ANON_KEY and SUPABASE_SERVICE_KEY?** ANON_KEY respects RLS — used by the app. SERVICE_KEY bypasses RLS entirely — superuser access. SERVICE_KEY is never put in the app because a compromised app would give an attacker unrestricted database access.
27. **How does auth.uid() work in Supabase RLS?** The app passes the farmer's Clerk JWT to Supabase. Supabase extracts auth.uid() from the JWT's `sub` claim. RLS policies compare auth.uid() against the row's producer_id. The database enforces ownership at row level.
28. **What is Just-In-Time (JIT) access?** Temporary, scoped, approved access for privileged operations (e.g. debugging prod data). Time-limited (minutes/hours not days), approved by a second person (four-eyes principle), every query logged to an immutable audit trail, auto-expires.
29. **Why are database migrations append-only?** When a migration runs, the database marks it as applied and never re-runs it. Editing a committed migration creates a lie in git history — the file says one thing, the database has another. New developers get a broken schema. Migrations are like audit_log: INSERT-only, never UPDATE.
30. **Why zero-pad migration numbers (001 not 1)?** Alphabetical sort order. Without padding, 10 sorts before 2 — migrations run in wrong order. 001, 002... 010 sorts correctly always.
31. **What is idempotency in migrations?** A migration that can be run multiple times without error or data change. Achieved with `CREATE TABLE IF NOT EXISTS` and `CREATE TYPE IF NOT EXISTS`. If a migration runner crashes and re-runs, no damage done.
32. **What is TIMESTAMPTZ vs TIMESTAMP?** TIMESTAMPTZ stores a moment in UTC and converts to the user's timezone on read. TIMESTAMP has no timezone awareness — a timestamp created in India looks like the same wall-clock time in Canada. Financial and legal timestamps must always be TIMESTAMPTZ.
33. **What is an AST (Abstract Syntax Tree)?** Python's parsed representation of source code as a tree of nodes. Every import, function, class is a node. The `ast` module lets you inspect any Python file's structure without executing it — used to scan domain/ for banned infrastructure imports.
34. **What are architecture fitness tests?** Tests that enforce structural rules of the codebase, not business logic. Examples: no infrastructure imports in domain/, no raw SQL in domain/, RLS on all tables. Run in CI on every push — violations block merges automatically.
