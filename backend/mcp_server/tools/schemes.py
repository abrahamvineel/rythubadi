...
@mcp.tool()
def get_schemes(province_state: str, crop_type: str) -> dict:
    try:
        validated = SchemesInput(province_state=province_state, crop_type=crop_type)
        schemes = _deps.scheme_repository.search(query=validated.crop_type,
                                                 regional_context=RegionalContext(validated.province_state),
                                                 top_k=5)
